"""주식 매수/매도 주문 TR 구조체 정의 (trio_ord.h c8101 기반)"""
...
class CTc8101InBlock(Structure):
    """주식 매수/매도 주문 입력 블록 C 구조체"""
    _fields_ = [
        ...
        ("shsll_pos_flagz1", ctypes.c_char * 1),        # 공매도가능여부
        ("_shsll_pos_flagz1", ctypes.c_char * 1),
...
class CTc8101OutBlock(Structure):
    """주식 매수/매도 주문 출력 블록 C 구조체"""
    _fields_ = [
...
class Tc8101InBlock(InBlock):
    ...
    C_STRUCT: ClassVar[Type[ctypes.Structure]] = CTc8101InBlock
    ...
    shsll_pos_flagz1: str = Field(max_length=1, description="공매도가능여부", default="0")
...
@dataclass
class Tc8101OutBlock(OutBlock):
    """주식 매수/매도 주문 출력 블록

    Attributes:
        order_noz10: 주문번호 (빈 값이면 주문 실패)
        order_qtyz12: 주문수량
        order_unit_pricez10: 주문단가
    """
